...
description: 在实现任何功能或修复 bug 时使用，编写实现代码之前
---
...
**核心原则：** 如果你没有观察测试失败，你就不知道它是否在测试正确的东西。
...
| 质量 | 好 | 坏 |
|---------|------|-----|
| **最小化** | 一件事。名称中有"和"？拆分开。 | `test('验证邮箱和域名和空格')` |
...
**"TDD 是教条的，做实事求是的意味着适应"**

TDD 是务实的：
- 在提交前发现 bug（比事后调试更快）
- 防止回归（测试立即捕获 break）
- 记录行为（测试展示如何使用代码）
- 支持重构（自由修改，测试捕获 break）

"务实"的捷径 = 在生产环境中调试 = 更慢。

**"之后写测试也能达到同样的目标——是精神不是仪式"**

不。之后写的测试回答"这是做什么的？"测试优先回答"这应该做什么？"

之后写的测试会被你的实现影响。你测试你构建的东西，而不是需要的东西。你验证记住的边界情况，而不是发现的边界情况。

测试优先强制在实现之前发现边界情况。之后写的测试验证你记住了一切（你没有）。

30 分钟的事后测试 ≠ TDD。你得到了覆盖率，失去了测试工作的证明。
...
| "保留作为参考，先写测试" | 你会调整它。那是事后测试。删除意味着删除。 |
| "需要先探索" | 可以。丢弃探索，用 TDD 开始。 |
| "测试难 = 设计不清晰" | 倾听测试。难测试 = 难使用。 |
| "TDD 会拖慢我" | TDD 比调试更快。实事求是 = 测试优先。 |
...
| 问题 | 解决方案 |
|--------|---------|
| 不知道如何测试 | 写出你期望的 API。先写断言。询问你的合作伙伴。 |
| 测试太复杂 | 设计太复杂。简化接口。 |
